...
export const verifyPhoneNumberOTP = async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (user.otp.expiresAt < new Date()) {
        return res.status(400).json({ message: "OTP has expired, kindly generate a new OTP" });
      }
      ...
      if (user.isPhoneNumberVerified) {
        return res.status(400).json({ message: "Your phone number is already verified" });
      }
  ...
  export const generateNewOTP = async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      return res.status(200).json({ message: "Kindly check your email for new OTP" });
    } catch (error) {
      return res.status(500).json({ message: "Server error", error }); 
    }
  };
